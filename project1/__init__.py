    ...
    def maximum_matching(self, keys_only=False):
        while True:
            ...
            for path in self._dfs_paths:
                ...
                self._nr_matches += 1
            print(self._nr_matches)
        return self._nr_matches
...
boxes = inputBoxes()
boxes = sorted(boxes)
graph = dict([((p,0),[(q,1) for q in filter(lambda x: fitsIn(p,x), boxes[i:])]) for i,p in enumerate(boxes) ])

hC = HopcroftKarp(graph)
print(len(boxes) - hC.maximum_matching())